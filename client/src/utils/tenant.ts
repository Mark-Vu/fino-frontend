...
export function getSubdomainFromUrl(): string | null {
    ...
    // For finotools.app, subdomain is the first part
    if (hostname.endsWith("finotools.app")) {
        if (parts.length >= 3) {
            return parts[0];
        }
        return null;
...
export function getMainDomainUrl(): string {
    if (typeof window === "undefined") return "https://finotools.app";
